...
def build_parcel_lookup() -> dict:
    ...
    try:
        ...
        while True:
            ...
            total  += len(features)
            offset += batch
            log.info(f"  Fetched {total} parcels so far...")
            if not data.get("exceededTransferLimit", False):
                break
        log.info(f"Collin CAD lookup built: {len(lookup):,} name variants from {total} parcels")
    except Exception:
...
def build_output(raw_records: list, date_from: str, date_to: str) -> dict:
    ...
    return {
        ...
        "source":       "Collin County TX – PublicSearch",
...
async def main():
    ...
    log.info("=== Collin County TX Lead Scraper ===")
